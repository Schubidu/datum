...
[![Build Status](https://secure.travis-ci.org/briannesbitt/Datum.png)](http://travis-ci.org/briannesbitt/Datum)
...
{{intro::exec(echo Datum::now()->subMinutes(2)->diffForHumans();)}} // '{{intro_eval}}'
...
```php
{{::lint($dtToronto = Datum::createFromDate(2012, 1, 1, 'America/Toronto');)}}
{{::lint($dtVancouver = Datum::createFromDate(2012, 1, 1, 'America/Vancouver');)}}
...
```php
{{::lint($carbon = new Datum();/*pad(40)*/)}} // equivalent to Datum::now()
{{::lint($carbon = new Datum('first day of January 2008', 'America/Vancouver');)}}
...
{{::lint($today = Datum::today();)}}
{{today::exec(echo $today;/*pad(40)*/)}} // {{today_eval}}
{{::lint($tomorrow = Datum::tomorrow('Europe/London');)}}
{{tomorrow::exec(echo $tomorrow;/*pad(40)*/)}} // {{tomorrow_eval}}
{{::lint($yesterday = Datum::yesterday();)}}
...
{{createFromDateException::exec(try { Datum::create(1975, 5, 21, 22, -2, 0); } catch(InvalidArgumentException $x) { echo $x->getMessage(); })}}
```
...
```php
{{createFromFormat1::exec(echo Datum::createFromFormat('Y-m-d H', '1975-05-21 22')->toDateTimeString();)}} // {{createFromFormat1_eval}}
```
...
```php
{{createFromTimeStamp1::exec(echo Datum::createFromTimeStamp(-1)->toDateTimeString();/*pad(80)*/)}} // {{createFromTimeStamp1_eval}}
{{createFromTimeStamp2::exec(echo Datum::createFromTimeStamp(-1, 'Europe/London')->toDateTimeString();/*pad(80)*/)}} // {{createFromTimeStamp2_eval}}
{{createFromTimeStampUTC::exec(echo Datum::createFromTimeStampUTC(-1)->toDateTimeString();/*pad(80)*/)}} // {{createFromTimeStampUTC_eval}}
```
...
```php
{{::lint($dt = Datum::create(2012, 9, 5, 23, 26, 11);)}}
...
{{getage::exec(var_dump(Datum::createFromDate(1975, 5, 21)->age);/*pad(54)*/)}} // {{getage_eval}} calculated vs now in the same tz
{{getq::exec(var_dump($dt->quarter);/*pad(54)*/)}} // {{getq_eval}}

// Returns an int of seconds difference from UTC (+/- sign included)
{{get1::exec(var_dump(Datum::createFromTimestampUTC(0)->offset);/*pad(54)*/)}} // {{get1_eval}}
{{get2::exec(var_dump(Datum::createFromTimestamp(0)->offset);/*pad(54)*/)}} // {{get2_eval}}

// Returns an int of hours difference from UTC (+/- sign included)
{{get3::exec(var_dump(Datum::createFromTimestamp(0)->offsetHours);/*pad(54)*/)}} // {{get3_eval}}

// Indicates if day light savings time is on
{{get4::exec(var_dump(Datum::createFromDate(2012, 1, 1)->dst);/*pad(54)*/)}} // {{get4_eval}}

// Gets the DateTimeZone instance
{{get5::exec(echo get_class(Datum::now()->timezone);/*pad(54)*/)}} // {{get5_eval}}
{{get6::exec(echo get_class(Datum::now()->tz);/*pad(54)*/)}} // {{get6_eval}}

// Gets the DateTimeZone instance name, shortcut for ->timezone->getName()
{{get7::exec(echo Datum::now()->timezoneName;/*pad(54)*/)}} // {{get7_eval}}
{{get8::exec(echo Datum::now()->tzName;/*pad(54)*/)}} // {{get8_eval}}
```
...
```php
{{isset1::exec(var_dump(isset(Datum::now()->iDoNotExist));/*pad(50)*/)}} // {{isset1_eval}}
{{isset2::exec(var_dump(isset(Datum::now()->hour));/*pad(50)*/)}} // {{isset2_eval}}
{{isset3::exec(var_dump(empty(Datum::now()->iDoNotExist));/*pad(50)*/)}} // {{isset3_eval}}
{{isset4::exec(var_dump(empty(Datum::now()->year));/*pad(50)*/)}} // {{isset4_eval}}
```
...
```php
{{::lint($dt = Datum::create(1975, 12, 25, 14, 15, 16);)}}
...
```php
{{::lint($first = Datum::create(2012, 9, 5, 23, 26, 11);)}}
{{::lint($second = Datum::create(2012, 9, 5, 20, 26, 11, 'America/Vancouver');)}}
...
```php
{{::lint($dt = Datum::create(2012, 1, 31, 0);)}}
...
{{::lint($dt = Datum::create(2012, 1, 31, 12, 0, 0);/*pad(40)*/)}}
{{addsub35::exec(echo $dt->startOfDay();/*pad(40)*/)}} // {{addsub35_eval}}

{{::lint($dt = Datum::create(2012, 1, 31, 12, 0, 0);)}}
...
{{diff1::exec(echo Datum::now('America/Vancouver')->diffInSeconds(Datum::now('Europe/London'));)}} // {{diff1_eval}}

{{::lint($dtOttawa = Datum::createFromDate(2000, 1, 1, 'America/Toronto');)}}
{{::lint($dtVancouver = Datum::createFromDate(2000, 1, 1, 'America/Vancouver');)}}
...
{{::lint($dt = Datum::create(2012, 4, 30, 0);)}}
...
{{::lint($dt = Datum::create(2012, 1, 1, 0);)}}
...
{{humandiff1::exec(echo Datum::now()->subDays(5)->diffForHumans();/*pad(62)*/)}} // {{humandiff1_eval}}

{{humandiff2::exec(echo Datum::now()->diffForHumans(Datum::now()->subYear());/*pad(62)*/)}} // {{humandiff2_eval}}

{{::lint($dt = Datum::createFromDate(2011, 2, 1);)}}
...
{{humandiff6::exec(echo Datum::now()->addSeconds(5)->diffForHumans();/*pad(62)*/)}} // {{humandiff6_eval}}
```
...
You can view the history of the Datum project in the [history file](https://github.com/briannesbitt/Datum/blob/master/history.md).

<a name="about-whyname"/>
### Why the name Datum?

Read about [Datum Dating](http://en.wikipedia.org/wiki/Radiocarbon_dating)